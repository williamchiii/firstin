...
function VoiceIntake({ onBack }) {
  ...
  const conversation = useConversation({
    ...
    onDisconnect: () => {
      if (hasConnectedRef.current) setPhase("ending");
    },
    onMessage: ({ role, message }) => {
      setMessages((prev) => [...prev, { role, message, id: Date.now() }]);
    },
    onError: (msg) => {
  ...
  useEffect(() => {
    ...
    const transcript = messages
      .map((m) => `${m.role === "ai" ? "Agent" : "Patient"}: ${m.message}`)
      .join("\n");
    ...
    return (
      <div className={`min-h-screen ${BG} flex flex-col`}>
        ...
        <div className="max-w-sm w-full flex flex-col items-center gap-6 text-center">
          ...
          <p className="text-gray-500 text-sm">
            ESI level <strong className="text-gray-800">{finalizeResult.esi}</strong> · Queue position{" "}
